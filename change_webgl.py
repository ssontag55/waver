...
asciiData = workspace + "wave_northamerica_webgl.txt";
...
newfile.write("[" +currentDate+", [");

#take first index
timestep = 1;
datapoints = 0;
fullDateList = currentDate;
...
for line in inlines:
	...
	if nullchecker.find(valnul,0) ==1:
		...
		if valuelist[0].strip() == currentDate:
			newfile.write(valuelist[2].strip()+','+str(float(valuelist[3])-360)+','+valuelist[4].replace("]","").strip());
			newfile.write(',');
			datapoints = datapoints+1;
		else:
			datapoints = 0;
			currentDate = valuelist[0].strip();
			fullDateList= fullDateList +','+currentDate;
